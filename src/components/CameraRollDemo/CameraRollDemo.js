...
export default class CameraRollDemo extends React.Component {
  ...
  onPictureSaved = async photo => {
    let uriPhoto= `${FileSystem.documentDirectory}photos/${Date.now()}.jpg`
    await FileSystem.moveAsync({
      from: photo.uri,
      to: uriPhoto,
    });
    this.props.extracTextFromImage(uriPhoto)
...
(
  ...
  //     return (
  //       <View style={{ flex: 1 }}>
  //         <Camera style={{ flex: 1,flexDirection: 'row', }} focusDepth={1} type={this.state.type} autoFocus='on' ref={ref => { this.camera = ref; }}>
  //             <TouchableOpacity
  //               style={{
  //                 flex: 1,
  //                 alignSelf: 'flex-end',
